function nextFocus($elNameNext){
	document.getElementById($elNameNext).focus();
}
...
DaftarObj2 = function(params_){
   ...
   this.bigImg = function() {
	...
	this.filterJenisSaldoChanged = function(){
		var me =this; //render sumhal

		$.ajax({
		  url: this.url+'&tipe=filterJenisSaldoChanged',
		  type:'POST',
			data:$('#'+this.formName).serialize(),
		  success: function(data) {
			var resp = eval('(' + data + ')');
			$("#filterBankKas").html(resp.content.filterBankKas);
			me.refreshList(true);
		  }
		});
	}
	this.filterBankKasChanged = function(){
		var me =this; //render sumhal
	...
	this.setTableHeader = function(){
			if(document.getElementById('spanFilterSaldo')){
				$.ajax({
					url: this.url+'&tipe=checkSisaSaldo',
					type:'POST',
					data:$('#'+this.formName).serialize(),
					success: function(data) {
					var resp = eval('(' + data + ')');
						$("#spanFilterSaldo").text(resp.content.spanFilterSaldo);
					}
				});
			}
...
// 	});
	...
	this.numberMark = function(obj){
		$("#"+obj.id).mask('000.000.000.000.000.000.000', {reverse: true});
	}
